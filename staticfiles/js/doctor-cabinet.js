...
async function loadDoctorProfile() {
    try {
        ...
        if (doctors.length > 0) {
            ...
            document.getElementById('cabinet-experience').textContent = `${doctor.experience_years} ${getYearsWord(doctor.experience_years)}`;
        ...
        if (dt <= now) {
            ...
            completeBtnHtml = `
                <span data-bs-toggle="tooltip" title="Прийом ще не розпочався" tabindex="0"
                      data-apt-time="${apt.date_time}" data-apt-id="${apt.id}" data-apt-patient="${escapeHtml(apt.patient_name)}">
...
function setupCompleteButtons() {
    document.querySelectorAll('.complete-btn:not([data-handler])').forEach(btn => {
 ...
 */
function startCompleteButtonTimer() {
    if (_cabinetTimer) clearInterval(_cabinetTimer);
    _cabinetTimer = setInterval(() => {
        const now = new Date();
        document.querySelectorAll('[data-apt-time]').forEach(span => {
            if (new Date(span.dataset.aptTime) <= now) {
                const id = span.dataset.aptId;
                const patient = span.dataset.aptPatient;
                const tooltipInstance = bootstrap.Tooltip.getInstance(span);
                if (tooltipInstance) tooltipInstance.dispose();
                const btn = document.createElement('button');
                btn.className = 'btn btn-sm btn-success complete-btn';
                btn.dataset.id = id;
                btn.dataset.patient = patient;
